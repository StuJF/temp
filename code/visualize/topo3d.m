...
EEG3dChannel = EEG3dset.EEG3dChannel;
rejChan = [];
for ic = 1:length(chanlocs)
    fname = chanlocs(ic).labels;
    for jc = 1:length(EEG3dChannel)
        if strcmpi(EEG3dChannel(jc).Name,fname)
            chanmark(ic,:) = EEG3dChannel(jc).Loc;
            rejChan = [rejChan,ic];
            break;
        end
...
plotchans = intersect(plotchans,rejChan);
