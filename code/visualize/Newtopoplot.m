...
set(ax2plot,'Xtick',[],'Ytick',[],'DataAspectRatio',[0.9,1,1],'xlim',[-450/k,450/k]*Rratio,...
    'Ylim',[-420/k,520/k]*Rratio,'clim',CLim);
axis off
set(ax2plot.Parent,'Color',[0.9 0.95 1]);
...
%             Zi = smooth2a(Zi,3,3);
        ...
        patch(ax2plot,ringx,ringy,zeros(size(ringx)),[0.9 0.95 1],'edgecolor','none');
    ...
    elseif size(Values,2)>1
        ...
        if isDir
            ...
            for ki = 1:size(ChanPair,1)
                plot(ax2plot,xx(ChanPair(ki,:)),yy(ChanPair(ki,:)),'LineWidth',LineWidth,...
                    'Color',color2plot(ki,:));
            end
        end
    end
